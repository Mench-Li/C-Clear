...
export function apply(ctx) {
  ...
  const PANEL_HTML = [
    ...
    'function log(t){var el=document.getElementById("log");if(el)el.textContent="["+new Date().toLocaleTimeString()+"] "+t+"\\n"+el.textContent}',
    'function el(id){return document.getElementById(id)}',
    'function stOf(p){return state.info[p]||{}}',
    'function subText(t){var s=stOf(t.path);if(s.status==="scan")return "统计中…";if(s.cleaned)return "已清理";var x=fmtB(s.sizeBytes);if(s.files!==undefined&&s.files!==null)x+=" · "+fmtC(s.files)+"文件";return x}',
    ...
    '  total=0;any=false;for(i=0;i<sel.length;i++){s=stOf(sel[i].path);if(s.sizeBytes!==undefined){total+=s.sizeBytes;any=true}}if(any)h+="<span class=\\"chip hot\\">合计 "+fmtB(total)+"</span>";',
